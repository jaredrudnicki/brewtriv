...
export function decryptQuestions(encrypted: any) {
    const decrypted = CryptoJS.AES.decrypt(encrypted, process.env.key);
    return JSON.parse(decrypted.toString(CryptoJS.enc.Utf8));
}

export function decryptCorrect(encrypted: any) {
    const decrypted = CryptoJS.AES.decrypt(encrypted, process.env.key).toString(
        CryptoJS.enc.Utf8,
    );
    return decrypted;
}
...
export async function addDaily(question: string, correct: string, incorrect: Array<string>) {
    ...
    const encCorrect = CryptoJS.AES.encrypt(
        correct,
        process.env.key,
    ).toString();

    const encIncorrect = encryptIncorrect(incorrect);
...
export async function getCustomerId(id: UUID | string) {
    const profile = await getProfile(id);
    if (profile) return profile.stripe_customer_id;
    return null;
}

export async function setCustomerId(id: UUID | string, customerId: string) {
    const supabase = createClient();
    const { error } = await supabase
        .from("profile_stats")
        .update({
...
export async function getSubscriptionId(id: UUID | string) {
    const profile = await getProfile(id);
    if (profile) return profile.stripe_subscription_id;
    return null;
}


export async function setSubscriptionId(id: UUID | string, subscriptionId: string) {
    const supabase = createClient();
    const { error } = await supabase
        .from("profile_stats")
        .update({
...
export async function getIsPremiumUser(id: UUID | string) {
